...
The S language was originally written by Jonathan Chambers in 1976(https://journal.r-project.org/articles/RJ-2020-028/RJ-2020-028.pdf), building upon the multics and BLISS (Bell Labs Interactive Statistical System) project efforts previous efforts involving John Tukey. Thus, from the start, the statistical language S was designed with interactive use in mind (hence the I in the name BLISS), thereby lending itself to smaller problems and exploratory data analysis that precedes statistical testing.   R language, derived from S, was first developed by Robert Gentleman and Ross Ihaka (https://doi.org/10.2307%2F1390807) and originally mentioned one of the key improvements was “memory management.” GPU computing derives its success in part by careful memory management that allows for rapid communication between threads within a streaming multiprocessor.
GPU computing is a key advantage of the most successful scripting languages for general use, though GPU use in R has not seen dramatic uptake. Surprisingly, the idea to use R with GPU capabilities is not new. The first GPU packages were written as far back as 2009, with the gputools package (https://pubmed.ncbi.nlm.nih.gov/19850754/) and NVIDIA training materials for R were made in 2014 (https://developer.nvidia.com/blog/accelerate-r-applications-cuda/).  The reasons for this include the small population size of packages on R, lack of maintenance, the typical R use case, and the CRAN platform’s rapid changes that makes package maintenance challenging over time. 
...
Several reasons exist why the adoption has been limited, including the three layer requirement for packages based on conda environments to use popular python GPU-interfaces and the incompatibility of R with older versions of packages. We believe the first could be solved by using CUDA C++ and C external wrapping functions with R (which an initial project has been built, ACF) and the latter could be solved by allowing a system for outdated R dependencies to be incorporated from the CRAN archive. 
